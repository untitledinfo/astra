...
<fieldset class="flex flex-col w-full {{ $divClass ?? '' }}">
    ...
    <textarea type="{{ $type ?? 'text' }}" id="{{ $id ?? $name }}" name="{{ $name }}"
        class="block w-full text-sm text-base bg-background-secondary border-2 border-neutral rounded-xl outline-none focus:outline-none focus:border-secondary transition-all duration-300 ease-in-out disabled:bg-background-secondary disabled:cursor-not-allowed {{ $class ?? '' }} @if ($type !== 'color') px-2.5 py-2.5 @endif"
